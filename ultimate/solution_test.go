...
func TestPipe_GracefulShutdownNextErrorAfterData(t *testing.T) {
	producer := &MockProducer{}
	consumer := &MockConsumer{}
	maxItems := 2

	// Next возвращает один пакет данных перед падением
	data1 := []any{"item1", "item2"}
	producer.On("Next").Return(data1, 1, nil).Once()

	data2 := []any{"item1", "item2"}
	producer.On("Next").Return(data2, 2, nil).Once()

	producer.On("Next").Return([]any{}, 0, errors.New("next failed")).Once()

	// Consumer должен обработать пакет данных
	consumer.On("Process", data1).Return(nil).Once()

	// Commit должен обработать cookie
	producer.On("Commit", 1).Return(nil).Once()
...
func TestPipe_GracefulShutdownMultipleStageErrors(t *testing.T) {
	producer := &MockProducer{}
	consumer := &MockConsumer{}
	maxItems := 2
	...
	data3 := []any{"item1", "item2"}
	producer.On("Next").Return(data3, 2, nil).Once()

	nextErr := errors.New("next stage failed")
	producer.On("Next").Return([]any{}, 0, nextErr).Once()

	consumer.On("Process", data1).Return(nil).Once()
	processErr := errors.New("process stage failed")
	consumer.On("Process", data2).Return(processErr).Once()

	commitErr := errors.New("commit stage failed")
	producer.On("Commit", 1).Return(commitErr).Once()

	err := Pipe(producer, consumer, maxItems)
	require.Error(t, err)
	require.Contains(t, err.Error(), nextErr.Error())
	require.Contains(t, err.Error(), processErr.Error())
	require.Contains(t, err.Error(), commitErr.Error())
